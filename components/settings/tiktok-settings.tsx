          ...
          components={{
            BottomSlot: (
              <div className="ml-8 flex items-center justify-between gap-3 border-slate-200/70 border-t pt-3 text-slate-500 text-xs dark:border-slate-600/60 dark:text-slate-400">
                ...
                <AppSwitch
                  aria-label="Show TikTok playback speed on page"
            ...
            RightSlot: (
              <AppSelect
                items={TIKTOK_SPEEDS.map((speed) => ({
                  label: `${speed}×`,
                  value: String(speed),
                }))}
                label="TikTok playback speed"
                onValueChange={(value) => onPlaybackSpeedChange(Number(value))}
                value={String(playbackSpeed)}
              />
            ),
          }}
          description="Remember the speed for TikTok videos"
          icon={GaugeIcon}
          ...
          components={{
            BottomSlot: (
              <div className="ml-8 flex items-center justify-between gap-3 border-slate-200/70 border-t pt-3 text-slate-500 text-xs dark:border-slate-600/60 dark:text-slate-400">
                <span>Show on page</span>
                <AppSwitch
                  aria-label="Show TikTok auto-skip on page"
